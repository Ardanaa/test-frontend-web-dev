<template>
  <main class="pa-9">
    ...
    <div v-if="isLoading" class="grid grid-cols-1 grid-rows-2 gap-2 md:grid-cols-3">
    ...
    <div v-if="isLoading" class="grid md:grid-rows-2 md:grid-flow-col grid-cols-1 md:grid-cols-3 gap-2">
      <div v-for="i in 4" :key="i" class="col-span-1 row-span-1 order-last md:order-first">
        <v-skeleton-loader
          class="mx-auto border"
          type="article"
        />
    ...
    <div v-else class="grid md:grid-rows-2 md:grid-flow-col grid-cols-1 md:grid-cols-3 gap-2">
      <div v-for="(news, index) in newsStore.restNewsUS" :key="index" class="col-span-1 row-span-1 order-last md:order-first">
        <CardB
          class="flex"
          v-bind="formatNews(news)"
        />
